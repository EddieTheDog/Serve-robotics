export async function onRequestPost({ request, env }) {
  const { token, guestId, reason } = await request.json();
  ...
  await env.DB.prepare(`
    INSERT INTO help_requests (id, guest_id, message, resolved, created_at) VALUES (?, ?, ?, 0, ?)
  `).bind(id, resolvedGuestId, reason ?? null, now).run();
...
export async function onRequestGet({ request, env }) {
  ...
  // Attendee polling: check if their help request was resolved
  if (token) {
    const guest = await env.DB.prepare(`SELECT id FROM guests WHERE app_token = ?`).bind(token).first();
    if (!guest) return Response.json({ resolved: false });
    const latest = await env.DB.prepare(`
      SELECT resolved FROM help_requests WHERE guest_id = ? ORDER BY created_at DESC LIMIT 1
    `).bind(guest.id).first();
    return Response.json({ resolved: latest?.resolved === 1 ?? false });
  }

  const query = unresolvedOnly
    ? `SELECT hr.id, hr.guest_id, hr.resolved, hr.created_at, hr.message, g.first_name, g.last_name, g.seat
...
export async function onRequestPatch({ request, env }) {
  ...
  // Reset guest status from 'help' back to 'accepted'
  const req = await env.DB.prepare(`SELECT guest_id FROM help_requests WHERE id = ?`).bind(id).first();
  if (req?.guest_id) {
